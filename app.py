...
def extract_year(set_name):
    match = re.search(r'\b\d{4}\b', set_name) ## grabs 4 consecutive integers from set name ##
...
## makes file uploader with the text in quotes ##
    ...
    if files: ## means if files=True which means if a file(s) is uploaded ##
        df_list = []
        for file in files:
            df_file = pd.read_csv(file)
            df_list.append(df_file) ## how to append multiple CSV's easily ##
        ...
        ## Idk yet but maybe dropdown selector thing for date? ##
        ...
        # and forgets the timestamp. ## 
            ...
            end_date = datetime.now().date() ## Gets current date and time, then .date() only gets date ##
            start_date = end_date - timedelta(days=6) ## Subtracts current day by 6 days to demonstrate the range of the last 7 Days ##
    ...
    ## makes NA in 'Purchase Price' either 2 or 0.5 depending on x ##
        ...
        #new_data_df['Acquisition Sold'] = pd.to_datetime(new_data_df['Acquisition Sold'], format='%m/%d/%Y %I:%M:$S %p')
    
        #new_data_df['Days to Sale'] = (new_data_df['Date Sold'] - new_data_df['Acquisition Sold']).dt.days

        #new_data_df['Annualized Return'] == new_data_df['Date Sold'] == 100 * (new_data_df['Sales Price'] - new_data_df['Purchase Price'] / new_data_df['Purchase Price']) * (365 * new_data_df['Days to Sale']) * 100
